...
fn test_binary_conn() {
    ...
    let mut rm = rustymem::new_with_protocol("127.0.0.1", P_BINARY);

    println( fmt!("versions: %?", rm.versions()) );

    println( fmt!("set_bytes key1 key1value: %?", rm.set_bytes("key1", 2*60*60, bytes!("key1value"))) );

    println( fmt!("add_bytes key1 key1value: %?", rm.add_bytes("key1", 60, bytes!("key1value"))) );
    println( fmt!("add_bytes key2 key2value: %?", rm.add_bytes("key2", 60, bytes!("key2value"))) );

    println( fmt!("replace_bytes key1 key1valueABC: %?", rm.replace_bytes("key1", 60, bytes!("key1valueABC"))) );
    println( fmt!("replace_bytes key_none key3value: %?", rm.replace_bytes("key_none", 60, bytes!("key3value"))) );
...
fn test_p_set(rm : &mut ~ProtoConnection) {
    println( fmt!("p_set: %?", rm.p_set("abc", bytes!("val123"), 0, 0, false)) );
    println( fmt!("p_set: %?", rm.p_set("abc", bytes!("val124"), 0, 0, true)) );

    let data = "xyz".to_owned();
    let data2 = data.into_bytes();
    println( fmt!("p_set: %?", rm.p_set("set1", data2, 0, 60, false)) );
}

fn test_p_add(rm : &mut ~ProtoConnection) {
    rm.p_add(&"add1", bytes!("xyz"), 0, 60, false);
}
...
fn test_rm(rm: &mut RustyMem) {
    ...
    // CAS data
    let md = rm.get_data("key1").unwrap();
    println( fmt!("get_data for cas %? : %?, %? %?", md.key, md.cas_unique, md, md.as_str()) );
    println( fmt!("cas_bytes key1 key1valueNotSet: %?", rm.cas_bytes("key1", 0, 0, bytes!("key1valueNotSet"))) );
    let md = rm.get_data("key1").unwrap();
    println( fmt!("get_data for cas %? : %?, %?", md.key, md.cas_unique, md.as_str()) );

    println( fmt!("cas_bytes key1 key1valueSet: %?", rm.cas_bytes("key1", md.cas_unique, 0, bytes!("key1valueSet"))) );
    let md = rm.get_data("key1").unwrap();
    println( fmt!("get_data for cas %? : %?, %?", md.key, md.cas_unique, md.as_str()) );
    ...
    let md = rm.get_data("key1").unwrap();
    println( fmt!("get_data %? : %?, %? %?", md.key, md.cas_unique, md, md.as_str()) );
...
fn main()  {
    ...
    //rustymem::memcached_client::memcached_client_hello();
